...
/// RAG pipeline for any type file(videos, images, audios) ////
const ragPipeline = async() => {
  try {
    // HOW TO HANDLE THE OTHER FILE TYPE
    // upload the file
    // pass a rag pipeline which is this function, receiving the file upload and a prompt
    // return details about the file uploaded from gemini analysis
    // vectorize the returned text
    // start chatting

   // Properties Of The Rag Pipeline
   // fetch the file given it id or something
   // upload it in the gemini file api(for large files)
   // put it in a format which the model can understand(convention)
   // return string of texts generated by the model
  } catch (error) {
