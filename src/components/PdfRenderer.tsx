...
const PdfRenderer = ({url}: PdfRendererProps) => {
  ...
  const CostumPageValidator = z.object({
    page: z.string().refine((num) => Number(num) > 0 && Number(num) <= numPages!)

  })

  type TCostumPageValidator = z.infer<typeof CostumPageValidator>

const { register, handleSubmit, formState: {errors}, setValue } = useForm<TCostumPageValidator>({
  defaultValues: {
    page: "1"
  }, 
  resolver: zodResolver(CostumPageValidator)
})

const handlePageSubmit = ({page}: TCostumPageValidator) => {
  ...
  return (
    <div className='w-full bg-white rounded-md flex flex-col items-center'>
      <div className="h-14 w-full border-b border-zic-200 flex items-center justify-between px-2">
        <div className="flex items-center gep-1 5">
          <Button 
          onClick={() => {
