...
export const ChatContexUrl = createContext<StreamResponseType>({
    addMessage: () => {},
    message: "",
    setMessage: () => {},
    handleInputChange: () => {},
...
export const ChatContextUrlProvider = ({fileId, children}: Props) => {
    ...
    const {mutate: sendMessage} = useMutation({
     ...
     onMutate: async({message}) => {
       ...
       setMessage("")
         ...
         ///// step 1////
        await utils.getUrlFileMessages.cancel()
 
         //// step 2 ////
         const prevoiusMessage = utils.getUrlFileMessages.getInfiniteData()
           
        
       //// step 3 /////
       utils.getUrlFileMessages.setInfiniteData(
         {urlfileId:fileId, limit: INFINITE_QUERY_LIMIT},
         (old) => {
            if(!old) {
             return {
                 pages: [],
     ...
     onSuccess: async(stream) => {
      ...
      while(!done){
         ...
         utils.getUrlFileMessages.setInfiniteData({urlfileId:fileId, limit: INFINITE_QUERY_LIMIT}, 
             (old:any) => {
     ...
     onError: ({_,__, context}) => {
         setIsLoading(false)
          setMessage(backupMessage.current)
           utils.getUrlFileMessages.setData(
             { urlfileId:fileId },
     ...
     onSettled: async() => {
        setIsLoading(false);
        await utils.getUrlFileMessages.invalidate({ urlfileId:fileId });
